    ...
    <div className="space-y-6">
            ...
            <tbody>
              {assets.map((a: any) => (
                <tr key={a.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3"><Badge variant={statusVariant[a.status] || 'secondary'}>{statusLabels[a.status] || a.status}</Badge></td>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="space-x-2">
          ...
          <Button variant="outline" size="sm" onClick={() => setPage(p => p + 1)} disabled={assets.length < 20}>下一页</Button>
        </div>
      </div>
    </div>
  );
